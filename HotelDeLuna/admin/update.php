...
if(isset($_POST['submit'])){
   ...
   if(!empty($admin_name)){
      $verify_admin_name = $conn->prepare("SELECT * FROM `admins` WHERE admin_name = ?");
      $verify_admin_name->execute([$name]);
      if($verify_admin_name->rowCount() > 0){
         $warning_msg[] = 'Username already taken!';
      }else{
         $update_admin_name = $conn->prepare("UPDATE `admins` SET admin_name = ? WHERE id = ?");
         $update_admin_name->execute([$name, $admin_id]);
