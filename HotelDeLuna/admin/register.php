...
if(isset($_POST['submit'])){
   ...
   $admin_name = filter_var($name, FILTER_SANITIZE_STRING); 
   $pass = sha1($_POST['pass']);
   $pass = filter_var($pass, FILTER_SANITIZE_STRING); 
   $c_pass = sha1($_POST['c_pass']);
   $c_pass = filter_var($c_pass, FILTER_SANITIZE_STRING);   

   $select_admins = $conn->prepare("SELECT * FROM `admins` WHERE admin_name = ?");
   $select_admins->execute([$admin_name]);
   ...
   if($select_admins->rowCount() > 0){
      ...
      if($pass != $c_pass){
         ...
         $insert_admin = $conn->prepare("INSERT INTO `admins`(id, admin_name, password) VALUES(?,?,?)");
         $insert_admin->execute([$id, $admin_name, $c_pass]);
